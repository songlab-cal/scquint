...
# https://raw.githubusercontent.com/alexdobin/STAR/master/extras/scripts/sjFromSAMcollapseUandM.awk
...
if (substr($1,1,1)!="@") {
    ...
    n=split($6,L,/[A-Z]/)-1;
    split($6,C,/[0-9]*/);
    # gbenegas: I don't see the value of variable t being used
    t=1;g=$4;
    ...
    for (k=1;k<=n;k++) {#scan through CIGAR operations
        if (C[k+1]=="S" || C[k+1]=="I") {
           ...
           sj1=$3 "\t" g "\t" g+L[k]-1;
           g+=L[k];
           if (L[k-1] < minOverhang || L[k+1] < minOverhang) continue;
           readSJs[sj1]++;
